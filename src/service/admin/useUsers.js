...
export const useUsers = () => {
  ...
  const [filter, setFilter] = useState(localStorage.getItem("level") || "xi");
  ...
  const getAbsoluteAllUserByLevel = async () => {
    const result = await usersQueries.getAbsoluteAllUsersByLevel(filter);
    setUsersAll(result);
  };
  ...
  const getUsersBy = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const result = await usersQueries.getUsersByLevel(filter, page, LIMIT, {
  ...
  const setFilterWithReset = useCallback(
    (newFilter) => {
      if (newFilter !== filter) {
        localStorage.setItem("level", newFilter);
        setFilter(newFilter);
        resetUsers();
      }
    },
