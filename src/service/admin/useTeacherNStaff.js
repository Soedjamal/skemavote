...
export const useTeacherNStaff = () => {
  const [users, setUsers] = useState([]);
  const [filter, setFilter] = useState(localStorage.getItem("levelT") || "ts");
  ...
  const getUsersBy = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const result = await usersQueries.getTeachersNStaffByLevel(
        filter,
        page,
        LIMIT,
        {
  ...
  const setFilterWithReset = useCallback(
    (newFilter) => {
      if (newFilter !== filter) {
        localStorage.setItem("levelT", newFilter);
        setFilter(newFilter);
        resetUsers();
      }
    },
